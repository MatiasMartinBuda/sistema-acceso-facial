...
# --- PARCHE DE RUTAS FORZADO ANTES DE IMPORTAR EL ENGINE ---
dir_proyecto = os.path.dirname(os.path.abspath(__file__))
# Forzamos a OpenCV a apuntar a tu carpeta local para buscar los XML
...
def pedir_datos():
    ...
    return dni, nombre, apellido, categoria, depto, tipo_acceso, pin
...
def capturar_rostros(label, engine: FaceEngine):
    ...
    while capturas < config.FOTOS_POR_ENROLAMIENTO:
        ...
        # Al haber aplicado el parche arriba, esta función ya no fallará internamente
...
def main():
    ...
    xml_rostro = os.path.join(dir_proyecto, "haarcascade_frontalface_default.xml")
    xml_ojo = os.path.join(dir_proyecto, "haarcascade_eye.xml")
    if not os.path.exists(xml_rostro) or not os.path.exists(xml_ojo):
        print("❌ ERROR crítico: Copiá los archivos 'haarcascade_frontalface_default.xml' y 'haarcascade_eye.xml'")
        print(f"dentro de la carpeta del proyecto antes de ejecutar: {dir_proyecto}")
        return

    engine = FaceEngine()

    dni, nombre, apellido, categoria, depto, tipo_acceso, pin = pedir_datos()
    label = database.alta_persona(dni, nombre, apellido, categoria, depto, tipo_acceso, pin)
